...
# REPO    
    ...
    REPO1="https://raw.githubusercontent.com/kytrx/v3/main/"
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
    ...
    wget -O /etc/haproxy/haproxy.cfg "${REPO1}xray/haproxy.cfg" >/dev/null 2>&1
...
wget -q -O /usr/local/sbin/quota "${REPO1}limit/quota"
chmod +x /usr/local/sbin/quota
chmod + x /usr/local/sbin/quota
cd /usr/local/sbin/
sed -i 's/\r//' quota
cd
wget -q -O /usr/bin/limit-ip "${REPO1}limit/limit-ip"
chmod +x /usr/bin/*
cd /usr/bin
sed -i 's/\r//' limit-ip
cd
...
wget -q -O /etc/ssh/sshd_config "${REPO1}ws/sshd" >/dev/null 2>&1
...
# // Installing Dropbear
 ...
 wget -q -O /etc/default/dropbear "${REPO1}ssh/dropbear.conf" && chmod +x /etc/default/dropbear
...
print_install "Menginstall ePro WebSocket Proxy"
    wget -O /usr/bin/ws "${REPO1}ws/ws" >/dev/null 2>&1
    wget -O /usr/bin/tun.conf "${REPO1}ws/tun.conf" >/dev/null 2>&1
    wget -O /etc/systemd/system/ws.service "${REPO1}ws/ws.service" >/dev/null 2>&1
...
wget -O /usr/sbin/ftvpn "${REPO1}ws/ftvpn" >/dev/null 2>&1
